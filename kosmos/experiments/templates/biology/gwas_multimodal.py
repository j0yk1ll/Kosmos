...
class GWASMultiModalTemplate(TemplateBase):
    ...
    def generate_protocol(self, params: TemplateCustomizationParams) -> ExperimentProtocol:
        ...
        variables = [
            Variable(
                name="snp_ids", description="List of SNP identifiers to analyze", value=snp_ids
            ),
            Variable(name="gene", description="Gene symbol for mechanism analysis", value=gene),
            Variable(
                name="gwas_path", description="Path to GWAS summary statistics CSV", value=gwas_path
            ),
            Variable(
                name="eqtl_path", description="Path to eQTL data CSV (optional)", value=eqtl_path
            ),
            Variable(
                name="pqtl_path", description="Path to pQTL data CSV (optional)", value=pqtl_path
            ),
            Variable(
                name="atac_path",
...
""",
        ...
        protocol = ExperimentProtocol(
            ...
            required_resources=ResourceRequirements(
                cpu_cores=2, memory_gb=4.0, storage_gb=2.0, estimated_duration_minutes=20
            ),
        )
    ...
    def _generate_load_data_code(
        self, gwas_path: str, eqtl_path: str | None, pqtl_path: str | None, atac_path: str | None
    ) -> str:
