...
class SystematicReviewTemplate(TemplateBase):
    ...
    def is_applicable(self, hypothesis: Hypothesis) -> bool:
        ...
        return (
            any(kw in statement for kw in keywords)
            and ExperimentType.LITERATURE_SYNTHESIS in hypothesis.suggested_experiment_types
        )
    ...
    def generate_protocol(self, params: TemplateCustomizationParams) -> ExperimentProtocol:
        ...
        steps = [
            ProtocolStep(
                ...
                description="Define search terms and databases",
                action="Create Boolean search query, select databases (arXiv, Semantic Scholar, PubMed), define inclusion/exclusion criteria",
            ...
            ProtocolStep(
                ...
                action="Run searches on all databases, combine results, remove duplicates, apply initial filters",
                expected_duration_minutes=180,
                requires_steps=[1],
                library_imports=["anthropic"],
            ),
            ProtocolStep(
                step_number=3,
                title="Screening and Selection",
                description="Screen papers for relevance",
                action="Title/abstract screening (first pass), full-text screening (second pass), document reasons for exclusion",
            ...
            ProtocolStep(
                ...
                description="Extract relevant data from selected papers",
                action="For each paper: extract methodology, findings, limitations, quality assessment (risk of bias), synthesize in structured format",
                expected_duration_minutes=400,
                requires_steps=[3],
                library_imports=["anthropic", "pandas"],
            ),
            ProtocolStep(
                step_number=5,
                title="Synthesis and Analysis",
                description="Synthesize findings across studies",
                action="Identify common themes, assess consistency of findings, note contradictions, synthesize conclusions",
            ...
            ProtocolStep(
                ...
                action="Write structured report: background, methods, results, discussion, gaps identified, conclusions",
                expected_duration_minutes=180,
                requires_steps=[5],
                library_imports=[],
            ),
        ...
        variables = {
            "included_papers": Variable(
                name="included_papers",
                type=VariableType.DEPENDENT,
                description="Number of papers meeting inclusion criteria",
                unit="count",
                measurement_method="Count after screening",
            ),
            "evidence_quality": Variable(
                name="evidence_quality",
                type=VariableType.DEPENDENT,
                description="Average quality score of included studies",
                unit="score_0_10",
                measurement_method="Risk of bias assessment",
            ),
        }
        ...
        resources = ResourceRequirements(
            ...
            api_calls_estimated=200,
            estimated_duration_days=5.0,
            required_libraries=["anthropic", "pandas"],
            python_version="3.9+",
    ...
    def generate_protocol(self, params: TemplateCustomizationParams) -> ExperimentProtocol:
        ...
        steps = [
            ProtocolStep(
                step_number=1,
                title="Literature Search",
                description="Systematic search for quantitative studies",
                action="Search databases for studies with numerical results, apply inclusion criteria (must report effect sizes or statistics to calculate them)",
                expected_duration_minutes=240,
                library_imports=["anthropic"],
            ),
            ProtocolStep(
                ...
                description="Extract effect sizes from studies",
                action="For each study: extract effect size (d, r, OR, etc.) or raw statistics, extract sample size, calculate SE, note study quality",
            ...
            ProtocolStep(
                ...
                description="Test for heterogeneity across studies",
                action="Calculate I² and Q statistics, assess between-study variance (τ²), decide on fixed vs random effects",
            ...
            ProtocolStep(
                step_number=4,
                title="Meta-Analysis Computation",
                description="Compute pooled effect size",
                action="Run random-effects meta-analysis, calculate pooled effect and CI, create forest plot",
                expected_duration_minutes=60,
                requires_steps=[3],
                library_imports=["scipy", "matplotlib"],
            ),
            ProtocolStep(
                step_number=5,
                title="Publication Bias Assessment",
                description="Check for publication bias",
                action="Create funnel plot, run Egger's test, conduct trim-and-fill analysis, assess selective reporting",
                expected_duration_minutes=45,
                requires_steps=[4],
                library_imports=["scipy", "matplotlib"],
            ),
            ProtocolStep(
                ...
                action="Leave-one-out analysis, subgroup analyses, meta-regression if enough studies",
        ...
        variables = {
            "effect_size": Variable(
                name="effect_size",
                type=VariableType.DEPENDENT,
                description="Standardized effect size from each study",
                unit="cohen_d_or_r",
                measurement_method="Extract from papers or calculate from statistics",
            ),
            "sample_size": Variable(
                name="sample_size",
                type=VariableType.INDEPENDENT,
                description="Sample size of each study",
                unit="count",
            ),
        }
        ...
        statistical_tests = [
            StatisticalTestSpec(
                ...
                description="Random-effects meta-analysis with DerSimonian-Laird estimator",
                null_hypothesis="H0: True effect size is zero",
        ...
        resources = ResourceRequirements(
            ...
            api_calls_estimated=100,
            estimated_duration_days=3.0,
            required_libraries=["scipy", "statsmodels", "matplotlib", "pandas", "numpy"],
            python_version="3.9+",
            can_parallelize=False,
        )
