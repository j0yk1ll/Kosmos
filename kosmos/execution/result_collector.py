...
from kosmos.db import (
    get_session,
    operations as db_ops,
)
...
class ResultCollector:
    ...
    def collect(
        ...
        statistical_tests: list[dict[str, Any]] | None = None,
        variable_data: dict[str, pd.Series] | None = None,
        ...
        supports_hypothesis = self._determine_hypothesis_support(
            primary_p_value, primary_effect_size, protocol
        )
        ...
        logger.info(
            f"Collected result for experiment {protocol.id}: "
    ...
    def _parse_stdout_data(self, stdout: str) -> dict[str, Any] | None:
    ...
    def _create_variable_results(
        self, variable_data: dict[str, pd.Series], protocol: ExperimentProtocol
    ) -> list[VariableResult]:
    ...
    ) -> tuple[list[StatisticalTestResult], str | None, float | None, float | None]:
        """
        ...
        for i, test_data in enumerate(statistical_tests):
            ...
            statistic = test_data.get(
                ...
                test_data.get(
                    ...
                    test_data.get(
                        "f_statistic",
                        test_data.get("u_statistic", test_data.get("chi2_statistic", 0.0)),
                    ),
                ),
            )
            ...
            # Significance
            sig_0_05 = test_data.get(
                "significant_0.05", test_data.get("significant_0_05", p_value < 0.05)
            )
            sig_0_01 = test_data.get(
                "significant_0.01", test_data.get("significant_0_01", p_value < 0.01)
            )
            sig_0_001 = test_data.get(
                "significant_0.001", test_data.get("significant_0_001", p_value < 0.001)
            )
            significance_label = test_data.get(
                "significance_label", self._get_significance_label(p_value)
            )
    ...
    def _determine_hypothesis_support(
        self, p_value: float | None, effect_size: float | None, protocol: ExperimentProtocol
    ) -> bool | None:
        ...
        try:
            ...
            with get_session() as session:
                db_result = db_ops.create_result(
                    ...
                    data=result_dict,
                    statistical_tests={
                        test.test_name: model_to_dict(test) for test in result.statistical_tests
                    },
    ...
    def export_result(
        self, result: ExperimentResult, format: str = "json", output_path: str | None = None
    ) -> str:
        """
    ...
    def create_version(
        ...
        """
        # Collect new result
        new_result = self.collect(protocol=protocol, execution_output=new_execution_output)
        ...
        logger.info(
            f"Created result version {new_result.version} " f"(parent: {original_result.id})"
        )
