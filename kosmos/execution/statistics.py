...
class StatisticalValidator:
    ...
    def calculate_cohens_d(
        group1: np.ndarray | list[float], group2: np.ndarray | list[float]
    ) -> float:
        """
    ...
    def parametric_confidence_interval(
        data: np.ndarray | list[float], confidence: float = 0.95
    ) -> tuple[float, float]:
        """
    ...
    def check_assumptions(
        data: np.ndarray | list[float], test_type: str = "t-test"
    ) -> dict[str, Any]:
        """
