...
class MLAnalyzer:
    ...
    def train_test_split_data(
        ...
        logger.info(
            f"Split data: train={len(X_train)}, test={len(X_test)} " f"(test_size={test_size})"
        )
    ...
    def cross_validate_model(
        ...
        logger.info(f"Cross-validation complete: {cv} folds, " f"stratified={stratified}")
    ...
    def evaluate_classification(
        ...
        logger.info(f"Classification evaluation: accuracy={accuracy:.4f}, " f"f1={f1:.4f}")
    ...
    def grid_search(
        ...
        logger.info(
            f"Grid search complete: tested {result['n_combinations_tested']} "
...
class FeatureEngineering:
    ...
    def create_polynomial_features(X: pd.DataFrame | np.ndarray, degree: int = 2) -> np.ndarray:
        ...
        logger.info(
            f"Created polynomial features: degree={degree}, "
    ...
    def select_k_best_features(
        ...
        if score_func not in score_funcs:
            raise ValueError(
                f"Unknown score_func '{score_func}'. " f"Use one of: {list(score_funcs.keys())}"
            )
