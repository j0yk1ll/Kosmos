...
class MemoryStore:
    ...
    def __init__(self, config: dict[str, Any] | None = None, max_memories: int = 1000):
        ...
        logger.info(
            f"MemoryStore initialized (max: {max_memories}, prune_after: {self.prune_after_days} days)"
        )
    ...
    def add_success_memory(
        self, result: ExperimentResult, hypothesis: Hypothesis, insights: str | None = None
    ) -> str:
        """
    ...
    def add_failure_memory(
        self, result: ExperimentResult, hypothesis: Hypothesis, failure_reason: str
    ) -> str:
        """
    ...
    def add_insight_memory(
        self, insight: str, source: str, related_hypotheses: list[str] | None = None
    ) -> str:
        """
    ...
    def query_memory(
        ...
        results.sort(
            ...
            * (1.0 / max(1, (datetime.utcnow() - m.created_at).days + 1)),
            reverse=True,
        )
    ...
    def record_experiment(
        self, hypothesis: Hypothesis, protocol: ExperimentProtocol | None = None
    ) -> str:
        """
    ...
    def is_duplicate_experiment(
        ...
        similar_hyp = sum(
            1
            for sig in self.experiment_signatures.values()
    ...
    def _prune_category(self, category: MemoryCategory):
        ...
        for memory in memories:
            should_keep = (
                memory.importance >= self.min_importance_to_keep
                or memory.created_at > cutoff_date
    ...
    def get_memory_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "by_category": {
                category.value: len(self.memories[category]) for category in MemoryCategory
            },
    ...
    def export_memories(self, category: MemoryCategory | None = None) -> list[dict[str, Any]]:
        """
