...
class CacheManager:
    ...
    def _initialize_caches(self):
        ...
        ttl_seconds = getattr(self._config.performance, "cache_ttl", 172800)  # 48 hours
    ...
    def get_cache(self, cache_type: CacheType) -> BaseCache | None:
        """
    ...
    def clear(self, cache_type: CacheType | None = None) -> dict[CacheType, int]:
        """
        Clear cache(s).
    ...
    def get_stats(self, cache_type: CacheType | None = None) -> dict[str, Any]:
        ...
        if total_requests > 0:
            all_stats["overall_hit_rate_percent"] = round(
                all_stats["total_hits"] / total_requests * 100, 2
            )
        else:
        ...
        for ctype, cache in self._caches.items():
            try:
                ...
                if hasattr(cache, "max_size_mb") and hasattr(cache, "cache_dir"):
                    cache_dir = Path(cache.cache_dir)
                    if cache_dir.exists():
                        total_mb = sum(f.stat().st_size for f in cache_dir.rglob("*.pkl")) / (
                            1024 * 1024
                        )
