...
class MaterialsConcept(BaseModel):
    ...
    external_ids: dict[str, str] = Field(
        default_factory=dict, description="External database IDs (Materials Project, AFLOW, etc.)"
    )
...
class MaterialsOntology:
    ...
    def _initialize_crystal_structures(self) -> None:
        ...
        for struct_id, name, desc in structures:
            self.add_concept(
                MaterialsConcept(
                    id=struct_id, name=name, type="crystal_structure", description=desc
                )
            )
    ...
    def _initialize_material_properties(self) -> None:
        ...
        electrical_props = [
            ...
            (
                ...
                "Ability to conduct electricity (S/m)",
            ),
        ...
        for prop_id, name, desc in electrical_props:
            self.add_concept(
                MaterialsConcept(id=prop_id, name=name, type="property", description=desc)
            )
        ...
        thermal_props = [
            ...
            (
                ...
                "Size change with temperature (K⁻¹)",
            ),
        ]
    ...
    def _initialize_materials_classes(self) -> None:
        ...
        for class_id, name, desc in materials_classes:
            self.add_concept(
                MaterialsConcept(id=class_id, name=name, type="material_class", description=desc)
            )
        ...
        semiconductor_types = [
            ...
            (
                ...
                "Conductive polymers and molecules",
            ),
        ]

        for semi_id, name, desc in semiconductor_types:
            self.add_concept(
                MaterialsConcept(id=semi_id, name=name, type="material_subclass", description=desc)
            )
    ...
    def _initialize_processing_methods(self) -> None:
        ...
        for proc_id, name, desc in processing_methods:
            self.add_concept(
                MaterialsConcept(id=proc_id, name=name, type="processing_method", description=desc)
            )
    ...
    def get_crystal_structures(self) -> list[MaterialsConcept]:
        ...
        return [
            concept for concept in self.concepts.values() if concept.type == "crystal_structure"
        ]

    def get_material_properties(self, category: str | None = None) -> list[MaterialsConcept]:
        """
        ...
        if category:
            ...
            for relation in self.relations:
                if (
                    relation.target_id == category_id
                    and relation.relation_type == MaterialsRelationType.PART_OF
                ):
            ...
            # Get all properties
            return [concept for concept in self.concepts.values() if concept.type == "property"]

    def get_processing_methods(self) -> list[MaterialsConcept]:
        """Get all processing methods"""
        return [
            concept for concept in self.concepts.values() if concept.type == "processing_method"
        ]
    ...
    def get_materials_by_class(self, material_class: str) -> list[MaterialsConcept]:
        ...
        for relation in self.relations:
            if (
                relation.target_id == material_class
                and relation.relation_type == MaterialsRelationType.IS_A
                and relation.source_id in self.concepts
                and self.concepts[relation.source_id].type == "material"
            ):
                material_ids.add(relation.source_id)

        return [self.concepts[mid] for mid in material_ids]

    def find_concept(self, query: str) -> MaterialsConcept | None:
        ...
        for concept in self.concepts.values():
            if concept.name.lower() == query_lower or query_lower in [
                s.lower() for s in concept.synonyms
            ]:
                return concept
    ...
    def get_related_concepts(
        self, concept_id: str, relation_type: MaterialsRelationType | None = None
