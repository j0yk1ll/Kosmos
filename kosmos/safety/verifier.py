...
class VerificationReport(BaseModel):
    ...
    def summary(self) -> str:
        ...
        if self.passed and not self.issues:
            return "✓ Verification passed (no issues)"

        error_count = len(self.get_issues_by_severity("error")) + len(
            self.get_issues_by_severity("critical")
        )
...
class ResultVerifier:
    ...
    def _detect_outliers(self, result: ExperimentResult) -> list[VerificationIssue]:
        ...
        for var_result in result.variable_results:
            if var_result.value is not None:
                values = (
                    var_result.value if isinstance(var_result.value, list) else [var_result.value]
                )
                try:
    ...
    def _validate_statistics(self, result: ExperimentResult) -> list[VerificationIssue]:
        ...
        if len(result.statistical_tests) > 1:
            ...
            has_correction = any(
                "bonferroni" in test.test_name.lower()
                or "fdr" in test.test_name.lower()
                or "holm" in test.test_name.lower()
                for test in result.statistical_tests
            )
    ...
    def cross_validate(
        self, original: ExperimentResult, replication: ExperimentResult
    ) -> VerificationReport:
        """
        ...
        if original.primary_effect_size is not None and replication.primary_effect_size is not None:
            ratio = abs(original.primary_effect_size) / (
                abs(replication.primary_effect_size) + 1e-10
            )
    ...
    def detect_errors(self, result: ExperimentResult) -> list[str]:
        ...
        if hasattr(result, "metadata") and result.metadata:
            error_msg = (
                result.metadata.error_message if hasattr(result.metadata, "error_message") else None
            )
            if error_msg:
