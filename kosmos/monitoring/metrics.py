...
class MetricsCollector:
    ...
    def __init__(self, registry: Any | None = None):
        ...
        self.cpu_usage_percent = Gauge(
            "kosmos_cpu_usage_percent", "CPU usage percentage", registry=self.registry
        )
        ...
        self.info = Info("kosmos_info", "Kosmos service information", registry=self.registry)
        self._set_info()
    ...
    # Research tracking
    def track_research_cycle(self, domain: str, status: str, duration: float | None = None):
    ...
    def track_experiment_complete(
        self, domain: str, experiment_type: str, status: str, duration: float
    ):
        ...
        self.experiments_total.labels(
            domain=domain, experiment_type=experiment_type, status=status
        ).inc()
    ...
    def track_api_call(
        ...
        if output_tokens > 0:
            self.api_tokens_total.labels(api=api, model=model, token_type="output").inc(
                output_tokens
            )
        if cost_usd > 0:
    ...
    def track_database_query(self, operation: str, table: str, status: str, duration: float):
        ...
        self.database_query_duration_seconds.labels(operation=operation, table=table).observe(
            duration
        )
