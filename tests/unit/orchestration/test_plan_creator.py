...
class TestResearchPlan:
    ...
    def test_basic_creation(self):
        ...
        plan = ResearchPlan(cycle=1, tasks=tasks, rationale="Test rationale", exploration_ratio=0.7)
    ...
    def test_to_dict(self):
        ...
        tasks = [Task(1, "data_analysis", "Task 1", "Output 1", [], True)]

        plan = ResearchPlan(cycle=5, tasks=tasks, rationale="Test rationale", exploration_ratio=0.5)
...
class TestPromptBuilding:
    ...
    def test_prompt_includes_recent_findings(self, plan_creator, sample_context):
        ...
        prompt = plan_creator._build_planning_prompt(
            research_objective="Test", context=sample_context, num_tasks=10, exploration_ratio=0.5
        )
...
class TestPlanRevision:
    ...
    async def test_revise_plan(self, plan_creator, sample_context):
        ...
        revised_plan = await plan_creator.revise_plan(
            original_plan, review_feedback, sample_context
        )
...
class TestPlanCreatorEdgeCases:
    ...
    async def test_create_plan_empty_context(self, plan_creator):
        ...
        plan = await plan_creator.create_plan(research_objective="Test", context={}, num_tasks=5)
    ...
    async def test_create_plan_single_task(self, plan_creator, sample_context):
        ...
        plan = await plan_creator.create_plan(
            research_objective="Test", context=sample_context, num_tasks=1
        )
