...
class TestResultCollection:
    ...
    def test_collect_successful_result(
        self, result_collector, sample_protocol, successful_execution_output
    ):
    ...
    def test_collect_failed_result(
        self, result_collector, sample_protocol, failed_execution_output
    ):
    ...
    def test_collect_with_statistical_tests(
        self,
        result_collector,
        sample_protocol,
        ...
        result = result_collector.collect(
            sample_protocol, successful_execution_output, statistical_tests=statistical_tests_output
        )
    ...
    def test_collect_with_variable_data(
        self, result_collector, sample_protocol, successful_execution_output, variable_data
    ):
        """Test collecting result with variable data."""
        result = result_collector.collect(
            sample_protocol, successful_execution_output, variable_data=variable_data
        )
...
class TestMetadataCreation:
    ...
    def test_create_metadata_basic(
        ...
        metadata = result_collector._create_metadata(
            sample_protocol, successful_execution_output, start_time, end_time
        )
    ...
    def test_metadata_includes_library_versions(
        self, result_collector, sample_protocol, successful_execution_output
    ):
        """Test metadata includes library versions."""
        metadata = result_collector._create_metadata(
            sample_protocol, successful_execution_output, datetime.utcnow(), datetime.utcnow()
        )
...
class TestVariableResultCreation:
    ...
    def test_variable_result_handles_missing_data(self, result_collector, sample_protocol):
        ...
        data_with_nan = {"var1": pd.Series([1, 2, np.nan, 4, 5])}
...
class TestHypothesisSupportDetermination:
    ...
    def test_hypothesis_supported_significant_and_large_effect(
        self, result_collector, sample_protocol
    ):
...
class TestResultExport:
    ...
    def test_export_result_markdown(
        self, result_collector, sample_protocol, successful_execution_output
    ):
...
class TestResultVersioning:
    ...
    def test_create_version(self, result_collector, sample_protocol, successful_execution_output):
        ...
        new_result = result_collector.create_version(
            original_result, new_execution_output, sample_protocol
        )
...
class TestDatabaseStorage:
    ...
    def test_store_result_not_called_when_disabled(
        self, result_collector, sample_protocol, successful_execution_output
    ):
