...
class TestDataAnalyzer:
        ...
        df = pd.DataFrame(
            {
                ...
                "value": np.concatenate(
                    [np.random.exponential(10, 30), np.random.exponential(15, 30)]  # skewed data
                ),
            }
        )

        result = DataAnalyzer.ttest_comparison(df, "group", "value", ("B", "A"), log_transform=True)
    ...
    def test_ttest_comparison_empty_group(self):
        ...
        df = pd.DataFrame({"group": ["A"] * 10, "value": np.random.randn(10)})
        ...
        df = pd.DataFrame(
            {
                "group": ["A"] * 100 + ["B"] * 100,
                "score": np.concatenate(
                    [np.random.normal(10, 1, 100), np.random.normal(15, 1, 100)]  # Large difference
                ),
            }
        )
    ...
    def test_correlation_analysis_with_nan(self):
        ...
        df = pd.DataFrame({"x": [1, 2, np.nan, 4, 5, 6], "y": [2, 4, 6, np.nan, 10, 12]})
    ...
    def test_log_log_scaling_removes_non_positive(self):
        ...
        df = pd.DataFrame({"x": [-1, 0, 1, 2, 3, 4, 5], "y": [10, 0, 2, 4, 6, 8, 10]})
...
class TestDataCleaner:
    ...
    def test_filter_positive(self):
        ...
        df = pd.DataFrame({"x": [-1, 0, 1, 2, 3], "y": [1, 2, 3, 4, 5]})
...
class TestDataAnalyzerIntegration:
        ...
        df = pd.DataFrame(
            {
                ...
                "response": np.concatenate(
                    [np.random.normal(100, 15, 50), np.random.normal(120, 15, 50)]
                ),
            }
        )
    ...
    def test_complete_correlation_workflow(self):
        ...
        # Analyze correlation
        result = DataAnalyzer.correlation_analysis(df_clean, "hours_studied", "exam_score")
    ...
    def test_complete_power_law_workflow(self):
        ...
        df = pd.DataFrame({"Length": neuron_length, "Synapses": synapse_count})
