...
def mock_components():
    ...
    mocks["skill_loader"].get_statistics.return_value = {"total_skills": 100}

    mocks["novelty_detector"].get_statistics.return_value = {"total_indexed_tasks": 0}
...
def mock_plan():
    ...
    plan = ResearchPlan(cycle=1, tasks=tasks, rationale="Test plan", exploration_ratio=0.7)
...
class TestResearchWorkflowInit:
    ...
    def test_components_initialized(self, temp_dir):
        ...
        workflow = ResearchWorkflow(
            research_objective="Test", artifacts_dir=str(temp_dir / "artifacts")
        )
...
class TestCycleExecution:
    ...
    async def test_execute_cycle(
        self, temp_dir, mock_plan, mock_review, mock_execution_result, mock_scholar_eval_score
    ):
        ...
        workflow.novelty_detector.check_plan_novelty = Mock(
            return_value={
        ...
        workflow.context_compressor.compress_cycle_results = Mock(
            return_value=Mock(summary="Compressed")
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_cycle_plan_rejected(self, temp_dir, mock_plan, mock_execution_result):
...
class TestResearchWorkflowStatistics:
    ...
    def test_compute_final_statistics(self, temp_dir):
        ...
        # Mock state manager
        workflow.state_manager.get_all_findings = Mock(
            return_value=[Mock(summary="Finding 1"), Mock(summary="Finding 2")]
        )
        workflow.state_manager.get_validated_findings = Mock(
            return_value=[Mock(summary="Finding 1")]
        )
...
class TestIntegrationPoints:
    ...
    @pytest.mark.asyncio
    async def test_novelty_detection_integration(
        self, temp_dir, mock_plan, mock_review, mock_execution_result, mock_scholar_eval_score
    ):
        ...
        # Add some past tasks
        workflow.past_tasks = [{"type": "data_analysis", "description": "Previous task"}]
...
class TestResearchWorkflowEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_single_task_per_cycle(
        self, temp_dir, mock_plan, mock_review, mock_execution_result, mock_scholar_eval_score
    ):
    ...
    def test_empty_research_objective(self, temp_dir):
        ...
        workflow = ResearchWorkflow(
            research_objective="", artifacts_dir=str(temp_dir / "artifacts")
        )
