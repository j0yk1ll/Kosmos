    ...
    def test_search_materials(self, mock_httpx_client):
        ...
        with patch("httpx.Client", return_value=mock_httpx_client):
            ...
            results = client.search_materials(
                elements=["Si"], band_gap_min=1.0, band_gap_max=2.0, limit=10
            )
...
class TestNOMADClient:
            ...
            "data": [
                {
                    "entry_id": "entry123",
                    "upload_id": "upload456",
                    "material": {"material_name": "Silicon", "chemical_formula_hill": "Si1"},
    ...
    def test_get_entry_data(self, mock_httpx_client):
        ...
        mock_response.json.return_value = {
            ...
            "material": {"material_name": "TiO2", "chemical_formula_hill": "O2Ti1"},
    ...
    def test_filtering_options(self, mock_httpx_client):
        ...
        with patch("httpx.Client", return_value=mock_httpx_client):
            client = NOMADClient()
            client.search_materials(elements=["Ti", "O"], data_type="experiment", limit=50)
...
class TestAflowClient:
    ...
    def test_properties_query(self, mock_httpx_client):
        ...
        mock_response = Mock()
        mock_response.json.return_value = [
            {"auid": "test", "compound": "GaN", "energy_atom": -3.45, "Egap": 3.4, "density": 6.15}
        ]
        mock_httpx_client.get.return_value = mock_response
    ...
    def test_batch_queries(self, mock_httpx_client):
        ...
        mock_response = Mock()
        mock_response.json.return_value = [
            {"auid": f"aflow:test{i}", "compound": f"Compound{i}"} for i in range(50)
        ]
        mock_httpx_client.get.return_value = mock_response
            ...
            "hits": [
                {
                    "_source": {
                        ...
                        "properties": [{"name": "battery_capacity", "scalars": [{"value": 170}]}],
                    }
                }
            ]
        }
...
class TestPerovskiteDBClient:
    ...
    def test_composition_queries(self):
        ...
        experiments = client.parse_experiments(
            df, composition_cols=["PbI2", "MAI"], fabrication_cols=["Pressure", "Temperature"]
        )
