...
def sample_expression_data():
    ...
    for i, gene_id in enumerate(gene_ids):
        if i < 20:  # Upregulated in disease
            counts_data[gene_id] = list(np.random.poisson(100, n_case)) + list(
                np.random.poisson(50, n_control)
            )
        elif i < 40:  # Downregulated in disease
            counts_data[gene_id] = list(np.random.poisson(50, n_case)) + list(
                np.random.poisson(100, n_control)
            )
    ...
    return {"counts": counts_matrix, "metadata": sample_metadata}
...
class TestDifferentialExpression:
    ...
    def test_de_analysis_disease_vs_control(
        self, neurodegeneration_analyzer, sample_expression_data
    ):
    ...
    def test_multiple_conditions(self, neurodegeneration_analyzer):
        ...
        # Create simple data
        counts = pd.DataFrame(
            {"Sample1": [100, 200, 50], "Sample2": [120, 180, 55], "Sample3": [80, 220, 45]},
            index=["GENE1", "GENE2", "GENE3"],
        )

        metadata = pd.DataFrame(
            {"sample_id": ["Sample1", "Sample2", "Sample3"], "disease": ["PD", "PD", "Healthy"]}
        )
...
class TestPathwayEnrichment:
    ...
    def test_gene_set_enrichment(self, neurodegeneration_analyzer, sample_expression_data):
        ...
        enrichment = neurodegeneration_analyzer.pathway_enrichment(
            deg_results=result, pathway_genes=pathway_genes, pathway_name="ECM Pathway"
        )
    ...
    def test_multiple_pathways(self, neurodegeneration_analyzer, sample_expression_data):
        ...
        for pathway_name, genes in pathways.items():
            enrich = neurodegeneration_analyzer.pathway_enrichment(result, genes, pathway_name)
    ...
    def test_significance_threshold(self, neurodegeneration_analyzer, sample_expression_data):
        ...
        pathway_genes = [f"GENE_{i}" for i in range(10)]
        enrichment = neurodegeneration_analyzer.pathway_enrichment(result, pathway_genes, "Test")
...
class TestCrossSpeciesValidation:
    ...
    def test_cross_species_comparison(self, neurodegeneration_analyzer):
        ...
        mouse_counts = pd.DataFrame(
            {"M1": [100, 200], "M2": [120, 180], "M3": [80, 220]}, index=["Gene1", "Gene2"]
        )

        mouse_metadata = pd.DataFrame(
            {"sample_id": ["M1", "M2", "M3"], "condition": ["Disease", "Disease", "Control"]}
        )
        ...
        human_counts = pd.DataFrame(
            {"H1": [110, 210], "H2": [130, 190], "H3": [85, 225]}, index=["Gene1", "Gene2"]
        )

        human_metadata = pd.DataFrame(
            {"sample_id": ["H1", "H2", "H3"], "condition": ["Disease", "Disease", "Control"]}
        )
        ...
        # Cross-species validation
        validation = neurodegeneration_analyzer.cross_species_validation(mouse_result, human_result)
    ...
    def test_conserved_genes_identification(self, neurodegeneration_analyzer):
        ...
        mouse_counts = pd.DataFrame(
            {"M1": [150, 50], "M2": [160, 55], "M3": [50, 150]}, index=["Gene1", "Gene2"]
        )
        ...
        human_counts = pd.DataFrame(
            {"H1": [155, 48], "H2": [165, 52], "H3": [48, 155]}, index=["Gene1", "Gene2"]
        )
    ...
    def test_validation_scoring(self, neurodegeneration_analyzer):
        ...
        mouse_counts = pd.DataFrame({"M1": [150], "M2": [160], "M3": [50]}, index=["Gene1"])
        ...
        human_counts = pd.DataFrame({"H1": [155], "H2": [165], "H3": [48]}, index=["Gene1"])
    ...
    def test_ortholog_mapping(self, neurodegeneration_analyzer):
        ...
        # Mouse data
        mouse_counts = pd.DataFrame(
            {"M1": [100], "M2": [120], "M3": [80]}, index=["Sod2"]
        )  # Mouse gene
        ...
        human_counts = pd.DataFrame(
            {"H1": [110], "H2": [130], "H3": [85]}, index=["SOD2"]
        )  # Human gene
    ...
    def test_species_specific_changes(self, neurodegeneration_analyzer):
        ...
        human_counts = pd.DataFrame(
            {"H1": [50, 150], "H2": [55, 160], "H3": [150, 50]}, index=["Gene1", "Gene2"]
        )
    ...
    def test_conservation_analysis(self, neurodegeneration_analyzer):
        ...
        human_counts = pd.DataFrame(
            {"H1": [105, 205], "H2": [125, 185], "H3": [82, 225]}, index=["Gene1", "Gene2"]
        )
