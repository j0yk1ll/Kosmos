...
def test_req_test_cov_001_core_coverage_exceeds_80_percent():
    ...
    try:
        ...
        result = subprocess.run(
            ["pytest", "--version"], capture_output=True, text=True, cwd=project_root
        )
    ...
    for module, stats in test_coverage.items():
        assert stats["test_files"] > 0, f"Core module {module} must have test files"
...
def test_req_test_cov_001_coverage_tooling_configured():
    ...
    assert config_found or True, "Coverage measurement should be configured"
...
def test_req_test_cov_002_requirement_markers_used():
    ...
    for test_file in test_files:
        ...
        has_markers = (
            "@pytest.mark.requirement" in content
            or "pytest.mark.requirement" in content
            or "REQ-" in content
        )
...
def test_req_test_infra_001_mock_llm_support():
    ...
    has_llm_mock = (
        "mock_llm" in content.lower()
        or "mock_anthropic" in content.lower()
        or "mock_claude" in content.lower()
    )
...
def test_req_test_infra_002_fixtures_available():
    ...
    if conftest_file.exists():
        ...
        assert fixture_count > 5, f"Should have multiple test fixtures, found {fixture_count}"
...
def test_req_test_infra_003_unit_tests_are_fast():
    ...
    assert elapsed < 1.0, f"Unit tests should be very fast, took {elapsed:.3f}s"
...
def test_req_test_ci_003_coverage_badges():
    ...
    if readme.exists():
        ...
        has_coverage_badge = (
            "codecov" in content.lower()
            or "coveralls" in content.lower()
