...
def test_req_data_001_supports_large_datasets(temp_dir):
    ...
    try:
        ...
        max_size = (
            loader.get_max_dataset_size() if hasattr(loader, "get_max_dataset_size") else None
        )
        ...
        assert hasattr(loader, "load_chunked") or hasattr(
...
def test_req_data_002_supports_json_format(temp_dir):
    ...
    data = {"records": [{"id": 1, "value": "a"}, {"id": 2, "value": "b"}, {"id": 3, "value": "c"}]}
...
def test_req_data_003_validates_schema(temp_dir):
    ...
    # Create test dataset
    csv_file = temp_dir / "test.csv"
    df = pd.DataFrame({"id": [1, 2, 3], "value": [1.5, 2.5, 3.5], "category": ["A", "B", "C"]})
    df.to_csv(csv_file, index=False)
    ...
    try:
        ...
        expected_schema = {
            ...
            "types": {"id": "int", "value": "float", "category": "str"},
        }
        ...
        assert (
            result.is_valid or result.passed
...
def test_req_data_003_detects_schema_mismatches(temp_dir):
    ...
    df = pd.DataFrame({"wrong_col1": [1, 2, 3], "wrong_col2": ["a", "b", "c"]})
    df.to_csv(csv_file, index=False)
    ...
    try:
        ...
        assert not result.is_valid, "Schema validation should fail for mismatched schema"
        ...
        assert expected_cols != actual_cols, "Should detect column mismatch"
...
def test_req_data_004_missing_value_detection(temp_dir):
    ...
    try:
        ...
        assert hasattr(quality_report, "missing_values") or hasattr(
...
def test_req_data_004_outlier_detection(temp_dir):
    ...
    df = pd.DataFrame({"value": [1, 2, 3, 4, 5, 100, 2, 3, 4, 5]})  # 100 is an outlier
    df.to_csv(csv_file, index=False)
...
def test_req_data_006_original_dataset_not_modified(temp_dir):
    ...
    original_data = pd.DataFrame({"id": [1, 2, 3], "value": [10, 20, 30]})
    ...
    try:
        ...
        assert original_content == current_content, "Original dataset file must not be modified"
...
def test_req_data_007_rejects_high_missing_values(temp_dir):
    ...
    try:
        ...
        assert (
            not result.is_valid or result.has_critical_issues
...
def test_req_data_008_no_automatic_data_mixing():
    ...
    try:
        ...
        assert (
            not hasattr(loader, "auto_merge") or not loader.auto_merge
        ...
        if hasattr(loader, "merge_datasets"):
            ...
            assert len(sig.parameters) > 1, "Merge should require explicit parameters"
...
def test_req_data_009_requires_dataset_provenance(temp_dir):
    ...
    try:
        ...
        if hasattr(result, "has_provenance"):
            assert not result.has_provenance, "Dataset without provenance should be flagged"
...
def test_req_data_010_011_enforces_5gb_limit(temp_dir):
    ...
    try:
        ...
        with patch("os.path.getsize", return_value=6 * 1024 * 1024 * 1024):  # 6GB
            ...
            assert not result.is_valid, "Validator should reject datasets >5GB"

            assert (
                "5GB" in str(result.error) or "size" in str(result.error).lower()
...
def test_req_data_012_rejects_raw_image_data(temp_dir):
    ...
    try:
        ...
        assert not result_image.is_valid, "Should reject raw image files"

        # Should reject sequencing files
        result_fastq = validator.validate(str(fastq_file))
        assert not result_fastq.is_valid, "Should reject raw sequencing files"
