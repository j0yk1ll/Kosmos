...
def test_req_meta_001_all_must_requirements_have_tests():
    ...
    if len(critical_reqs) > 0:
        ...
        if uncovered_reqs:
            ...
            for req in sorted(uncovered_reqs)[:10]:
                print(f"    - {req}")
...
def test_req_meta_001_requirement_markers_properly_used():
    ...
    for test_file in test_files:
        ...
        proper_markers = len(
            re.findall(r'@pytest\.mark\.requirement\(["\']REQ-[A-Z]+-\d+["\']\)', content)
        )
...
def test_req_meta_002_production_readiness_criteria():
    ...
    if requirements_file.exists():
        ...
        has_prod_criteria = "production" in content.lower() and "ready" in content.lower()

        assert has_prod_criteria, "Production readiness criteria should be documented"

        # Check for specific criteria
        criteria_keywords = ["all.*must.*pass", "test.*pass", "coverage", "validation"]

        import re

        criteria_found = sum(
            1 for pattern in criteria_keywords if re.search(pattern, content.lower())
        )
...
def test_req_meta_003_should_requirements_tracked():
    ...
    if len(should_reqs) > 0:
        ...
        print("SHOULD requirement test coverage:")
...
class TestMetaRequirementsIntegration:
    ...
    def test_production_readiness_documentation(self):
        ...
        has_prod_section = (
            "Production Readiness" in content or "production-ready" in content.lower()
        )

        assert has_prod_section, "Production readiness criteria must be documented"
        ...
        found_criteria = sum(1 for pattern in criteria if re.search(pattern, content.lower()))
    ...
    def test_continuous_testing_configured(self):
        ...
        if pytest_ini.exists():
            ...
            ci_friendly = (
                "junit" in content.lower()
                or "xml" in content.lower()
                or "verbose" in content.lower()
            )
