...
def test_req_domain_001_three_domains_supported():
    ...
    with patch.dict(
        ...
        assert (
            len(actual_domains.intersection(expected_domains)) >= 3
...
def test_req_domain_001_domain_workflow_execution():
    ...
    try:
        ...
        for domain in test_domains:
            try:
                ...
                workflow = ResearchWorkflow(
                    research_question=f"Sample {domain} research question", domain=domain
                )
        ...
        for domain in test_domains:
            with patch.dict(
                os.environ, {"ANTHROPIC_API_KEY": "test_key", "ENABLED_DOMAINS": domain}
            ):
                try:
                    config = get_config(reload=True)
        ...
        assert successful_configs >= 3, "At least 3 domains must be configurable"
...
def test_req_domain_002_unified_interface():
    ...
    try:
        ...
        for query, _expected_domain in test_queries:
            ...
            assert detected is not None, f"Router should handle query: {query[:50]}"

            # Should use configuration, not hardcoded logic
            assert hasattr(router, "domain_patterns") or hasattr(
                router, "domain_keywords"
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test_key"}):
            ...
            assert isinstance(
                config.research.enabled_domains, list
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.If):
                        ...
                        assert not any(
                            d in condition_str.lower()
                            for d in ["biology", "neuroscience", "physics"]
...
def test_req_domain_003_domain_templates_exist():
    ...
    for location in template_locations:
        ...
        for domain in ["biology", "neuroscience", "physics", "chemistry", "materials"]:
            ...
            if domain_dir.exists() and domain_dir.is_dir():
                ...
                template_files = (
                    list(domain_dir.glob("*.py"))
                    + list(domain_dir.glob("*.yaml"))
                    + list(domain_dir.glob("*.json"))
                    + list(domain_dir.glob("*.md"))
                )
                if template_files:
    ...
    if len(templates_found) >= 3:
        ...
        print(
            f"⚠ Domain templates should be provided for at least 3 domains, found {len(templates_found)}"
        )

    # This is a SHOULD requirement, so we don't fail but log
    assert len(templates_found) >= 0, "Domain template infrastructure should exist"
...
def test_req_domain_003_domain_specific_prompts():
    ...
    with patch.dict(
        os.environ,
        {"ANTHROPIC_API_KEY": "test_key", "ENABLED_DOMAINS": "biology,neuroscience,materials"},
    ):
        config = get_config(reload=True)

        # Check if system supports domain-specific prompts
        has_domain_prompts = (
            hasattr(config, "prompts")
            or hasattr(config, "domain_templates")
            or hasattr(config.research, "domain_specific_prompts")
        )
...
class TestDomainIntegration:
    ...
    def test_domain_agnostic_core_components(self):
        ...
        for domain_config in test_configs:
            ...
            with patch.dict(
                os.environ, {"ANTHROPIC_API_KEY": "test_key", "ENABLED_DOMAINS": domain_config}
            ):
                config = get_config(reload=True)
