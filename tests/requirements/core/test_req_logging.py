...
class TestREQ_LOG_002_ConfigurableLogLevels:
    ...
    def test_all_standard_log_levels_supported(self):
        ...
        from kosmos.core.logging import LogFormat, setup_logging
        ...
        for level in log_levels:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                logger = setup_logging(level=level, log_format=LogFormat.TEXT, log_file=log_file)
    ...
    def test_log_level_from_config(self):
        ...
        with patch.dict(
            os.environ,
            {"ANTHROPIC_API_KEY": "sk-ant-log-level", "LOG_LEVEL": "ERROR", "LOG_FORMAT": "json"},
        ):
...
class TestREQ_LOG_003_PersistentStorage:
    ...
    def test_log_rotation_configuration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger = setup_logging(level="INFO", log_format=LogFormat.JSON, log_file=log_file)
...
class TestREQ_LOG_005_NoSensitiveInfo:
    ...
    def test_password_redaction(self):
        ...
        def sanitize_log_message(message: str) -> str:
            ...
            # Redact passwords
            message = re.sub(
                r"password[=:]\s*\S+", "password=[REDACTED]", message, flags=re.IGNORECASE
            )

            # Redact API keys
            message = re.sub(
                r"(api[_-]?key[=:]\s*)\S+", r"\1[REDACTED]", message, flags=re.IGNORECASE
            )
    ...
    def test_database_credentials_not_logged(self, caplog):
        ...
        with patch.dict(
            ...
            with caplog.at_level(logging.INFO):
                ...
                for record in caplog.records:
                    assert (
                        "password" not in record.message.lower()
                        or "[REDACTED]" in record.message
                        or "password@localhost" not in record.message
                    )
...
class TestREQ_LOG_006_CorrelationIDs:
    ...
    def test_correlation_id_in_logs(self, caplog):
        ...
        from kosmos.core.logging import ExperimentLogger, LogFormat, setup_logging
...
class TestLoggingIntegration:
    ...
    def test_complete_logging_workflow(self):
        ...
        from kosmos.core.logging import ExperimentLogger, LogFormat, get_logger, setup_logging
