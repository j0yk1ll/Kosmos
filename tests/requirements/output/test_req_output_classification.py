    ...
    try:
        ...
        for claim in claims:
            classification = classifier.classify(
                claim_text=claim["text"], source=claim["source"], context=claim["context"]
            )
            classified_claims.append({**claim, "classification": classification})

        # Assert: All claims are classified
        assert len(classified_claims) == len(claims), "All claims should be classified"

        unclassified = [c for c in classified_claims if c["classification"] is None]
        assert len(unclassified) == 0, "No claims should be unclassified"

        # Assert: Classifications use three defined categories
        valid_types = {
            StatementType.DATA_ANALYSIS,
            StatementType.LITERATURE,
            StatementType.INTERPRETATION,
        }

        for claim in classified_claims:
            assert (
                claim["classification"] in valid_types
        ...
        assert len(classification_types) == 3, "Should classify into all three categories"

        # Assert: Data analysis claims identified
        data_analysis_claims = [
            c for c in classified_claims if c["classification"] == StatementType.DATA_ANALYSIS
        ]
        assert len(data_analysis_claims) == 2, "Should identify 2 data analysis claims"

        # Assert: Literature claims identified
        literature_claims = [
            c for c in classified_claims if c["classification"] == StatementType.LITERATURE
        ]
        assert len(literature_claims) == 2, "Should identify 2 literature claims"

        # Assert: Interpretation claims identified
        interpretation_claims = [
            c for c in classified_claims if c["classification"] == StatementType.INTERPRETATION
        ]
        assert len(interpretation_claims) == 2, "Should identify 2 interpretation/synthesis claims"
        ...
        def classify_claim(claim_text: str, source: str) -> str:
            ...
            if any(
                indicator in claim_text.lower()
                for indicator in ["correlation", "p<", "p=", "significant", "fdr", "coefficient"]
            ):
            ...
            if any(
                indicator in claim_text.lower()
                for indicator in ["et al.", "previous studies", "research shows", "established"]
            ):
            ...
            if any(
                indicator in claim_text.lower()
                for indicator in ["suggest", "indicates", "implies", "convergence", "potential"]
            ):
        ...
        for text, source, expected_type in test_claims:
            ...
            assert classification == expected_type, f"Should classify '{text}' as {expected_type}"
...
def test_req_out_class_001_classification_features():
    ...
    try:
        ...
        assert data_features["source_type"] == "notebook", "Data analysis source should be notebook"

        # Assert: Literature features
        assert lit_features["has_citation"], "Literature claim should have citations"
        assert lit_features[
            "has_author_year_pattern"
        ], "Literature claim should have author-year pattern"

        # Assert: Interpretation features
        assert interp_features[
            "has_synthesis_language"
        ], "Interpretation claim should have synthesis language"
        assert interp_features[
            "has_tentative_language"
        ...
        def extract_features(claim: dict) -> dict:
            ...
            features = {
                ...
                "has_synthesis_language": any(
                    word in text for word in ["suggest", "indicate", "imply", "collectively"]
                ),
                "has_definitive_language": any(
                    word in text for word in ["shows", "demonstrates", "found", "revealed"]
                ),
            }
...
def test_req_out_class_002_provenance_with_type():
    ...
    try:
        ...
        report = generator.generate_report(title="Classification Test Report", workflow_id="wf_001")
        ...
        for claim in report.claims:
            ...
            assert "statement_type" in provenance or hasattr(
        ...
        for claim in report["claims"]:
            assert "statement_type" in claim["provenance"], "Provenance must include statement_type"
            assert (
                "expected_accuracy" in claim["provenance"]
...
def test_req_out_class_002_accuracy_validation_by_type():
    ...
    try:
        ...
        assert 0.60 <= overall_accuracy <= 0.70, "Overall accuracy should be intermediate value"
