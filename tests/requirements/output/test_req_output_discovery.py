...
def test_req_out_disc_001_identify_narratives():
    ...
    try:
        ...
        narratives = identifier.identify_narratives(world_model=world_model, workflow_id="wf_001")
                ...
                narratives.append(
                    {
                        ...
                        "coherence_score": len({kw for f in theme_findings for kw in f["keywords"]})
...
def test_req_out_disc_001_coherence_scoring():
    ...
    try:
        ...
        assert coherent_score > 0.5, "Highly related findings should score >0.5 coherence"
    ...
    try:
        ...
        for narrative in narratives:
            for finding_id in narrative.core_findings:
                assert (
                    finding_id not in core_finding_assignments
        ...
        assert (
            len(narratives_referencing_bridge) >= 1
        ...
        for finding in findings:
            ...
            if "secondary_theme" in finding:
                ...
                if finding["secondary_theme"] in narratives:
                    narratives[finding["secondary_theme"]]["bridge_findings"].append(finding["id"])
            else:
                # Core finding
        ...
        assert len(all_core_findings) == len(
            set(all_core_findings)
...
def test_req_out_disc_001_minimal_narrative_size():
    ...
    try:
        ...
        assert len(narratives) == 2, "Only clusters with >=3 findings should become narratives"
        ...
        assert not any(
            "f8" in n.findings and len(n.findings) < 3 for n in narratives
        ...
        valid_narratives = {
            name: findings for name, findings in clusters.items() if len(findings) >= MIN_FINDINGS
        }
