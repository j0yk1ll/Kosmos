...
def test_req_daa_sum_001_natural_language_summaries():
    ...
    try:
        ...
        assert "correlation" in summary.lower(), "Summary should mention the analysis type"
        assert "0.85" in summary or "85" in summary, "Summary should include key results"

        # Verify it's descriptive prose, not just data dump
        assert any(
            word in summary.lower()
            for word in ["found", "shows", "indicates", "reveals", "analysis"]
...
def test_req_daa_sum_002_statistical_findings_included():
    ...
    try:
        ...
        assert "p" in summary.lower() and (
            "value" in summary.lower() or "=" in summary
        ), "Summary should include p-value"

        assert (
            "0.0012" in summary or "0.001" in summary or "p < 0.01" in summary
        ), "Summary should report the p-value"

        # Check for effect size
        assert (
            "effect" in summary.lower() or "cohen" in summary.lower() or "0.67" in summary
        ), "Summary should include effect size"

        # Check for confidence interval
        assert (
            "confidence" in summary.lower() or "CI" in summary or "95%" in summary
        ), "Summary should mention confidence intervals"

        # Check for means comparison
        assert (
            "10.5" in summary or "mean" in summary.lower()
...
def test_req_daa_sum_003_serialize_sessions():
    ...
    """
    from kosmos.agents.data_analysis_agent import AnalysisSession
