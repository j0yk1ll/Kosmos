...
class TestREQ_PERF_SCALE_001_FortyThousandLinesCapacity:
    ...
    def test_forty_thousand_lines_capacity(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test code capacity", config={"max_iterations": 100}
        )
        ...
        assert total_lines >= 40000, f"Only tracked {total_lines} lines, need 40K+"

        # Verify performance
        assert duration < 60, f"Processing 40K lines took {duration:.1f}s, too slow"

        # Verify memory efficiency
        assert memory_used < 500, f"Used {memory_used:.1f}MB for 40K lines, too much"
    ...
    def test_code_line_tracking_accuracy(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test tracking accuracy", config={"max_iterations": 10}
        )
        ...
        # Calculate total
        total_lines = sum(director.research_plan.metadata["code_lines_by_experiment"].values())
    ...
    def test_large_codebase_performance(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test large codebase", config={"max_iterations": 50}
        )
        ...
        if len(operation_times) > 5:
            ...
            if first_half_avg > 0:
                ...
                assert slowdown < 2.0, f"Performance degraded {slowdown:.1f}x with larger codebase"
...
class TestREQ_PERF_SCALE_002_ThousandPapersCapacity:
    ...
    def test_thousand_papers_capacity(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test paper capacity", config={"max_iterations": 100}
        )
        ...
        assert paper_count >= 1000, f"Only stored {paper_count} papers, need 1000+"

        # Verify performance
        assert duration < 30, f"Adding 1000 papers took {duration:.1f}s, too slow"

        # Verify memory efficiency (should be < 100MB for metadata)
        assert memory_used < 200, f"Used {memory_used:.1f}MB for 1000 papers, too much"
    ...
    def test_paper_retrieval_performance(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test paper retrieval", config={"max_iterations": 10}
        )
        ...
        assert avg_retrieval_time < 0.001, f"Average retrieval {avg_retrieval_time:.6f}s too slow"

        assert max_retrieval_time < 0.01, f"Max retrieval {max_retrieval_time:.6f}s too slow"
    ...
    def test_paper_citation_tracking(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test citations", config={"max_iterations": 10}
        )
        ...
        for i in range(1200):
            ...
            cited_papers = [f"paper_{(i + j + 1) % 1200}" for j in range(3 + (i % 3))]
        ...
        assert total_papers >= 1000, f"Only {total_papers} papers tracked"

        # Verify citation counts
        total_citations = sum(
            len(cites) for cites in director.research_plan.metadata["citations"].values()
        )
        assert total_citations > 3600, "Should have many citations for 1200 papers"
    ...
    def test_large_literature_review_simulation(self):
        ...
        # Verify scale
        assert papers_analyzed >= 1000, f"Only analyzed {papers_analyzed} papers"

        # Verify organization
        assert len(papers_by_topic) == 5
        for topic, papers in papers_by_topic.items():
            assert len(papers) >= 200, f"Topic {topic} has only {len(papers)} papers"
...
class TestREQ_PERF_SCALE_003_HundredFiftyRolloutsCapacity:
    ...
    def test_hundred_fifty_rollouts_capacity(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test rollout capacity", config={"max_iterations": 50}
        )
        ...
        assert slowdown < 2.0, f"Performance degraded {slowdown:.1f}x over 150 rollouts"

        # Verify memory efficiency
        assert memory_used < 300, f"Used {memory_used:.1f}MB for 150 rollouts, too much"
    ...
    def test_rollout_state_consistency(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test state consistency", config={"max_iterations": 50}
        )
    ...
    def test_concurrent_rollout_processing(self, mock_wm, mock_llm):
        ...
        assert duration < 60, f"Batch processing took {duration:.1f}s, too slow"
    ...
    def test_rollout_metrics_tracking(self):
        ...
        for i in range(150):
            ...
            # Track completion
            metrics.record_experiment_end(f"exp_{i}", duration_seconds=0.001, status="success")
    ...
    def test_rollout_data_structure_efficiency(self):
        ...
        assert size_per_rollout < 1000, f"Each rollout uses {size_per_rollout:.0f} bytes, too much"
    ...
    def test_rollout_cleanup_efficiency(self, mock_wm, mock_llm):
        ...
        director = ResearchDirectorAgent(
            research_question="Test cleanup", config={"max_iterations": 50}
        )
        ...
        assert retention_percent < 20, f"{retention_percent:.1f}% of memory retained after cleanup"
