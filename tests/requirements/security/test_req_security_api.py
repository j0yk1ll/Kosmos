...
def test_req_sec_api_001_credentials_from_environment():
    ...
    try:
        ...
        assert config.api_key == test_key, "API key should be loaded from environment variable"
                    ...
                    if matches:
                        ...
                        for match in matches:
                            ...
                            hardcoded_keys_found.append(
                                {"file": str(py_file), "match": match[:50]}  # First 50 chars
                            )
...
def test_req_sec_api_002_rate_limiting_exists():
    ...
    try:
        ...
        assert hasattr(config, "llm_rate_limit_per_minute") or hasattr(
            config.research, "llm_rate_limit_per_minute"
        ...
        if rate_limit is not None:
            ...
            assert rate_limit <= 1000, "Rate limit should be reasonable (not unlimited)"
...
def test_req_sec_api_003_response_validation():
    ...
    class APIResponseValidator:
        ...
        @staticmethod
        def validate_response(response: dict, expected_schema: dict = None) -> dict:
            """
            ...
            if len(response_str) > APIResponseValidator.MAX_RESPONSE_SIZE:
                errors.append(f"Response too large: {len(response_str)} bytes")
    ...
    valid_response = {"id": "123", "content": "Safe content", "metadata": {"version": "1.0"}}
    ...
    assert len(result["warnings"]) > 0, "Suspicious content should trigger warnings"
...
def test_req_sec_api_003_json_validation():
    ...
    def validate_json_response(raw_response: str, max_size: int = 1024 * 1024) -> dict:
        """
        ...
        if len(raw_response) > max_size:
            return {"valid": False, "error": "Response too large", "data": None}
...
def test_req_sec_api_004_no_user_data_without_consent():
    ...
    class ConsentManager:
        ...
        def grant_consent(self, user_id: str, purpose: str, data_types: list) -> str:
    ...
    consent_id = consent_mgr.grant_consent(
        user_id=user_id, purpose="llm_analysis", data_types=["research_data", "metadata"]
    )

    # Assert: Consent should be granted
    assert consent_mgr.has_consent(
        user_id, "llm_analysis", "research_data"
    ), "Should have consent for research data"

    # Assert: Should not have consent for other data types
    assert not consent_mgr.has_consent(
        user_id, "llm_analysis", "personal_info"
...
def test_req_sec_api_004_data_minimization():
    ...
    def minimize_data_for_api(data: dict, required_fields: list, pii_fields: list = None) -> dict:
        """
        ...
        minimized = {key: data[key] for key in required_fields if key in data}
    ...
    assert "metadata" not in minimized, "Non-required fields should not be included"
...
def test_req_sec_api_005_no_sensitive_cache_plaintext():
    ...
    class SecureCache:
        ...
        def _is_sensitive(self, data: dict) -> bool:
            ...
            return any(keyword in data_str for keyword in self.sensitive_keywords)
        ...
        def set(self, key: str, value: dict, encrypt: bool = None):
            ...
            if encrypt:
                ...
                self.cache[cache_key] = {"data": encrypted_value, "encrypted": True}
            else:
                # Store plaintext for non-sensitive data
                self.cache[cache_key] = {"data": value, "encrypted": False}
    ...
    non_sensitive = {"result": "success", "data": [1, 2, 3, 4, 5], "timestamp": "2024-01-01"}

    cache.set("query1", non_sensitive)
    retrieved = cache.get("query1")

    # Assert: Non-sensitive data should be retrievable
    assert retrieved == non_sensitive, "Non-sensitive data should be cached and retrieved"
    ...
    assert cached_entry["encrypted"], "Sensitive data should be encrypted"

    assert isinstance(cached_entry["data"], bytes), "Encrypted data should be bytes"

    # Assert: Plaintext sensitive data should not be in cache
    assert (
        b"sk-secret-key-12345" not in cached_entry["data"]
    ), "API key should not be visible in encrypted cache"

    # Assert: Decryption should work
    retrieved_sensitive = cache.get("query2")
    assert retrieved_sensitive == sensitive, "Should be able to decrypt and retrieve sensitive data"
...
def test_req_sec_api_005_cache_expiration():
    ...
    assert cache.get("short_lived") is not None, "Should retrieve non-expired data"

    # Wait for expiration
    time.sleep(1.1)

    # Assert: Should be None after expiration
    assert cache.get("short_lived") is None, "Should not retrieve expired data"
