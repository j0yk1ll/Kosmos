...
def upgrade() -> None:
    ...
    op.create_index("idx_experiments_domain", "experiments", ["domain"], unique=False)

    op.create_index("idx_experiments_hypothesis_id", "experiments", ["hypothesis_id"], unique=False)

    op.create_index("idx_experiments_created_at", "experiments", ["created_at"], unique=False)

    # Composite indexes for common query patterns
    op.create_index(
        "idx_experiments_domain_status", "experiments", ["domain", "status"], unique=False
    )

    op.create_index(
        "idx_experiments_status_created", "experiments", ["status", "created_at"], unique=False
    )
    ...
    # Single-column indexes
    op.create_index("idx_hypotheses_domain", "hypotheses", ["domain"], unique=False)

    op.create_index("idx_hypotheses_status", "hypotheses", ["status"], unique=False)

    op.create_index("idx_hypotheses_novelty_score", "hypotheses", ["novelty_score"], unique=False)

    op.create_index(
        "idx_hypotheses_testability_score", "hypotheses", ["testability_score"], unique=False
    )

    op.create_index("idx_hypotheses_created_at", "hypotheses", ["created_at"], unique=False)

    # Composite indexes for common query patterns
    op.create_index(
        "idx_hypotheses_domain_status", "hypotheses", ["domain", "status"], unique=False
    )

    op.create_index(
        "idx_hypotheses_domain_novelty", "hypotheses", ["domain", "novelty_score"], unique=False
    )
    ...
    # Single-column indexes
    op.create_index("idx_papers_domain", "papers", ["domain"], unique=False)

    op.create_index("idx_papers_source", "papers", ["source"], unique=False)

    op.create_index("idx_papers_relevance_score", "papers", ["relevance_score"], unique=False)

    op.create_index("idx_papers_publication_date", "papers", ["publication_date"], unique=False)

    op.create_index("idx_papers_created_at", "papers", ["created_at"], unique=False)

    # Composite indexes for common query patterns
    op.create_index(
        "idx_papers_domain_relevance", "papers", ["domain", "relevance_score"], unique=False
    )

    op.create_index(
        "idx_papers_domain_pubdate", "papers", ["domain", "publication_date"], unique=False
    )
    ...
    # Single-column indexes
    op.create_index("idx_results_experiment_id", "results", ["experiment_id"], unique=False)

    op.create_index("idx_results_p_value", "results", ["p_value"], unique=False)

    op.create_index(
        "idx_results_supports_hypothesis", "results", ["supports_hypothesis"], unique=False
    )
    ...
    op.create_index("idx_research_sessions_domain", "research_sessions", ["domain"], unique=False)

    op.create_index(
        "idx_research_sessions_created_at", "research_sessions", ["created_at"], unique=False
    )
